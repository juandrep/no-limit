...
export function AudioVisualizer({
  ...
  const animationRef = useRef<number | null>(null); // Fixed: explicitly typed with null
  ...
  const draw = useCallback(() => {
    ...
    if (primaryColor) {
      ...
      resolvedColorWithOpacity = isDark
        ? "rgba(96, 165, 250, 0.7)"
        : "rgba(59, 130, 246, 0.5)";
    }
    ...
    if (analyser && audioContext && isPlaying) {
      ...
      for (let i = 0; i < barCount; i++) {
        const barHeight = isPlaying
          ? (Math.sin(time * 2 + i * 0.5) * 0.5 + 0.5) * canvas.height * 0.3 +
            10
          : Math.random() * 15 + 5;
  ...
  useEffect(() => {
    if (isPlaying) {
      ...
      if (animationRef.current) {
        ...
        animationRef.current = null; // Clear the ref after cancellation
      }
